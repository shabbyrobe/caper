...
if ($options['records']) {
    foreach ($options['<file>'] as $file) {
        ...
        foreach ($parser->recordIterator($handle, $query) as list($trace, $record)) {
            $out = (array) $record;
            $out['trace'] = $trace->id;
...
elseif ($options['pop']) {
    foreach ($options['<file>'] as $file) {
        $handle = fopen_arg($file, 'r');
        foreach ($parser->stackIterator($handle) as list($traceId, $popped, $stack)) {
            $out = (array) $popped;
            $out['trace'] = $traceId;
            $out['stack'] = $stack;
            $out['file']  = $file;
